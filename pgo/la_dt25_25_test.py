"""Grid-search LA quantile and scale for TUSREC25 / DualTrack25.

Example:
    python pgo/pose_graph_optimization/la_dt25_25_test.py \
        -c pgo/experiments/la_replace/dt25/tusrec25/config.yaml
"""
...
from pose_graph_optimization.utils import inbetween_to_accumulated  # noqa: E402
...
def evaluate_combination(
    ...
    for scan in scans:
        ...
        drift_metrics.append(
            get_drift_metrics(torch.as_tensor(scan["gt_acc"]), pred_acc)
        )
        ddf_metrics.append(
            get_ddf_metrics(
                pred_acc,
...
def save_best_plots(
    ...
    for scan_index, scan in enumerate(scans):
        ...
        optimized_acc = inbetween_to_accumulated(pred_inbetween)
        fig = plot_trajectories(
            ...
            labels=["GT", "Initial estimated", "LA"],
        ...
        output_path = output_dir / f"la_replace_best_{metric_name}_scan_{scan_index}.png"
...
def main() -> None:
    ...
    output_path = args.output or args.config.parent / "la_grid_search.csv"

    print(f"Loaded {len(scans)} scans")
    print(f"Testing {len(GRID_VALUES) ** 2} quantile/scale combinations")
