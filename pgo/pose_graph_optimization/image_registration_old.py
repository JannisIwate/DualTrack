...
def register(
    ...
    # How similar are base and registered image statistically? (negative, smaller is better)
    registration.SetMetricAsMattesMutualInformation(numberOfHistogramBins=50)

    # Means squares over pixel intensities (lower is better)
    # negative(?) mean squares (zero is best)
    # -> discard registrations which are too far off compared to ref frames
    #registration.SetMetricAsMeanSquares()

    # How correlated are base and registered image? (-1 to 1, negative, so smaller is better!)
    # -> discard registrations which are too far off compared to ref frames
    ...
    #     )
    # )
    print(f"score before reg: {registration.MetricEvaluate(fixed, moving)}")
    # register
    T_px_reg = registration.Execute(fixed, moving)
    print(f"score after reg: {registration.GetMetricValue()}")
